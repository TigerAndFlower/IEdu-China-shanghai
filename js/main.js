...
$('body').on('click', '#applySubmit, #bottomApplySubmit, #rightApplySubmit', function () {
    ...
    var content = form.serialize() + '&activity_id=104&huodong_af=' + getQueryString('tgfrom');
...
// 显示报名弹窗
$('body').on('click', '.showApplyBtn, .QA-school-btn span, .book, .linkTosignUp', function () {
    $('.popup').show();
    $('.popup-apply').show();
    $('.apply-success').hide();
});
...
$("#ccBox").on("click", function () {
    console.log(11)
    if($(".btn-min").eq(1).css("display","none")){
        console.log(22)
        // $(".btn-max")[0].attr("display",'none');
        // $(".btn-min")[0].removeAttr("display",'none');
        // $(".m-lite-content")[0].removeAttr("display",'none');
        // $(".m-lite-tip")[0].removeAttr("display",'none');
        // $(".m-lite-editor")[0].removeAttr("display",'none');
        // $(".m-lite-opt")[0].removeAttr("display",'none');
        // $(".m-lite-preview")[0].removeAttr("display",'none');
        $(".btn-max").eq(1).css("display","none");
        $(".btn-min").eq(1).css("display","block");
        $(".m-lite-content").eq(0).css("display","block");
        $(".m-lite-tip").eq(0).css("display","block");
        $(".m-lite-editor").eq(0).css("display","block");
        $(".m-lite-opt").eq(0).css("display","block");
        $(".m-lite-preview").eq(0).css("display","block");
    }
})
